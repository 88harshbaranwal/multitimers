...
function startNewTimer() {
  ...
  const total_in_seconds = (parseInt(hours) * 360) + (parseInt(minutes) * 60) + (parseInt(seconds));
  createTimer(total_in_seconds);
...
function createTimer(time_in_seconds) {
  ...
  activeTimers.appendChild(stopButton);
  activeTimers.appendChild(deleteButton);
